...
sys_exofork(void)
{
   ...
   memmove(
      (void *)&childenv->env_tf,
      (const void *)&curenv->env_tf,
...
//   panic("in sys_env_set_pgfault_upcall, env_id = %d, env_pgfault_upcall = %8x \n\n\n", env->env_id, func);
   return 0;
	// LAB 4: Your code here.
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
   if (
      (srcva >= (void *)UTOP) ||
      (srcva != (void *)ROUNDDOWN(srcva, PGSIZE)) ||
      (dstva >= (void *)UTOP) ||
      (dstva != (void *)ROUNDDOWN(dstva, PGSIZE))
   ) {
      return -E_INVAL;
   }
...
{
   if ((va >= (void *)UTOP) || (va != (void *)ROUNDDOWN(va, PGSIZE))) {
      return -E_INVAL;
   }
   struct Env *env;
   int r;
